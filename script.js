...
function generateTables() {
  ...
  for (const code in serviceData) {
    ...
    serviceData[code].forEach((item, i) => {
      ...
      monthInput.oninput = () => autoCalcWeek(code, i);
...
function autoCalcMonth(code, i) {
  const weekInput = document.getElementById(`${code}_week_${i}`);
  const monthInput = document.getElementById(`${code}_month_${i}`);

  let week = parseFloat(weekInput.value);
  if (isNaN(week) || week < 0) week = 0;

  const month = week * WEEKS_PER_MONTH;

  monthInput.value = month.toFixed(1);
  calcTotal(code, i);
}

// 每月次數變動時計算每週次數並更新總價
function autoCalcWeek(code, i) {
  const weekInput = document.getElementById(`${code}_week_${i}`);
  const monthInput = document.getElementById(`${code}_month_${i}`);
  ...
  const week = month / WEEKS_PER_MONTH;

  weekInput.value = week.toFixed(1);
  calcTotal(code, i);
}
...
function calcTotal(code, i) {
  ...
  const monthInput = document.getElementById(`${code}_month_${i}`);
  const totalCell = document.getElementById(`${code}_total_${i}`);
  ...
  totalCell.textContent = total.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
...
function calculate() {
  ...
  for (const code in serviceData) {
    ...
    for (let i = 0; i < serviceData[code].length; i++) {
      const totalText = document.getElementById(`${code}_total_${i}`).textContent;
      const totalValue = parseFloat(totalText.replace(/,/g, '')) || 0;
      subTotal += totalValue;
    }
    detail += `<li>${code}碼合計：${subTotal.toLocaleString()} 元</li>`;
    totalAll += subTotal;
  }
